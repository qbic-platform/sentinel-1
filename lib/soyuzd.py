...
class SoyuzDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some soyuzd methods
    ...
    @classmethod
    def from_soyuz_conf(self, soyuz_dot_conf):
        from soyuz_config import SoyuzConfig
        config_text = SoyuzConfig.slurp_config_file(soyuz_dot_conf)
        creds = SoyuzConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = soyuzlib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = soyuzlib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = soyuzlib.blocks_to_seconds(diff)
    ...
    def has_sentinel_ping(self):
        ...
        return (getinfo['protocolversion'] >= config.min_soyuzd_proto_version_with_sentinel_ping)
