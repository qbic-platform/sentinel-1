...
def has_soyuz_conf():
    ...
    valid_soyuz_conf = False

    # ensure soyuz_conf exists & readable
    #
    # if not, print a message stating that Soyuz Core must be installed and
    # configured, including JSONRPC access in soyuz.conf
    try:
        f = io.open(config.soyuz_conf)
        valid_soyuz_conf = True
    except IOError as e:
        print(e)

    return valid_soyuz_conf
...
def main():
    ...
    if not has_soyuz_conf():
        print("SoyuzCore must be installed and configured, including JSONRPC access in soyuz.conf")
        sys.exit(1)
