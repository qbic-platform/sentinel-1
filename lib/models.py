...
class GovernanceObject(BaseModel):
    ...
    @classmethod
    def sync(self, soyuzd):
        golist = soyuzd.rpc_command('gobject', 'list')
        ...
        try:
            ...
            for item in golist.values():
                (go, subobj) = self.import_gobject_from_soyuzd(soyuzd, item)
    ...
    @classmethod
    def import_gobject_from_soyuzd(self, soyuzd, rec):
        import decimal
        import soyuzlib
        import inflection
        ...
        # shim/soyuzd conversion
        object_hex = soyuzlib.SHIM_deserialise_from_soyuzd(object_hex)
        objects = soyuzlib.deserialise(object_hex)
        subobj = None
        ...
        # exclude any invalid model data from soyuzd...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from soyuzd, with every run
        ...
        # get/create, then sync payment amounts, etc. from soyuzd - Soyuzd is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(soyuzd)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from soyuzd! %s" % e)
            govobj.vote_delete(soyuzd)
            return (govobj, None)
    ...
    def vote_delete(self, soyuzd):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(soyuzd, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, soyuzd, signal, outcome):
        import soyuzlib
        ...
        output = soyuzd.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = soyuzlib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(soyuzd, signal)

    def sync_network_vote(self, soyuzd, signal):
        printdbg('\tsyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = soyuzd.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 soyuz addr, non-multisig
            if not soyuzlib.is_valid_soyuz_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid Soyuz address for network [%s], returning False" % (self.payment_address, config.network))
                return False
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (soyuzlib.blocks_to_seconds(superblockcycle) / 2) +
    ...
    def is_deletable(self):
        ...
        # TBD (item moved to external storage/SoyuzDrive, etc.)
        return False
    ...
    def get_prepare_command(self):
        import soyuzlib
        obj_data = soyuzlib.SHIM_serialise_for_soyuzd(self.serialise())
    ...
    def prepare(self, soyuzd):
        try:
            object_hash = soyuzd.rpc_command(*self.get_prepare_command())
...
class Superblock(BaseModel, GovernanceClass):
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not soyuzlib.is_valid_soyuz_address(addr, config.network):
    ...
    def hash(self):
        import soyuzlib
        return soyuzlib.hashit(self.serialise())
...
class Watchdog(BaseModel, GovernanceClass):
    ...
    @classmethod
    def active(self, soyuzd):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at >= (now - soyuzd.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
        return resultset

    @classmethod
    def expired(self, soyuzd):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at < (now - soyuzd.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
    ...
    def is_expired(self, soyuzd):
        now = int(time.time())
        return (self.created_at < (now - soyuzd.SENTINEL_WATCHDOG_MAX_SECONDS))

    def is_valid(self, soyuzd):
        if self.is_expired(soyuzd):
            return False

        return True

    def is_deletable(self, soyuzd):
        if self.is_expired(soyuzd):
            return True
