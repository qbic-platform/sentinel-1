...
class SoyuzConfig():
    ...
    def slurp_config_file(self, filename):
        # read soyuz.conf config but skip commented lines
        f = io.open(filename)
        lines = []
        for line in f:
            if re.match('^\s*#', line):
        ...
        # data is soyuz.conf without commented lines
        data = ''.join(lines)
    ...
    def get_rpc_creds(self, data, network='mainnet'):
        # get rpc info from soyuz.conf
        match = re.findall(r'rpc(user|password|port)=(.*?)$', data, re.MULTILINE)

        # python >= 2.7
        creds = {key: value for (key, value) in match}

        # standard Soyuz defaults...
        default_port = 1918 if (network == 'mainnet') else 1917

        # use default port for network if not specified in soyuz.conf
        if not ('port' in creds):
            creds[u'port'] = default_port

        # convert to an int if taken from soyuz.conf
