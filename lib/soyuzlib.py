...
def is_valid_soyuz_address(address, network='mainnet'):
    ...
    soyuz_version = 112 if network == 'testnet' else 38
    ...
    try:
        ...
        # rescue from exception, not a valid Soyuz address
        return False

    if (address_version != soyuz_version):
        return False
...
# shims 'til we can fix the soyuzd side
def SHIM_serialise_for_soyuzd(sentinel_hex):
    ...
    # superblock => "trigger" in soyuzd
    if govtype == 'superblock':
        obj[0] = 'trigger'

    # soyuzd expects an array (even though there is only a 1:1 relationship between govobj->class)
    obj = [obj]

    # re-pack
    soyuzd_hex = serialise(obj)
    return soyuzd_hex


# shims 'til we can fix the soyuzd side
def SHIM_deserialise_from_soyuzd(soyuzd_hex):
    from models import DASHD_GOVOBJ_TYPES

    # unpack
    obj = deserialise(soyuzd_hex)
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('soyuz.conf')
