...
@pytest.fixture
def valid_soyuz_address(network='mainnet'):
    return 'n7mzEvBy6JdP2QQUqJrLjitucQKgTBZAR2' if (network == 'testnet') else 'GaoLZRViqSiELPkrdRabXrbYaT83hBZkrB'


@pytest.fixture
def invalid_soyuz_address(network='mainnet'):
    return 'n7mzEvBy6JdP2QQUqJrLjitu1QKgTBZFZ3' if (network == 'testnet') else 'GaoLZRViqSiELPkrdRabXrbYaT83hBZkrZ'
...
def mn_status_good():
    ...
    status = {
        ...
        "pubkey": "GaoLZRViqSiELPkrdRabXrbYaT83hBZkrB",
...
def test_valid_soyuz_address():
    from soyuzlib import is_valid_soyuz_address

    main = valid_soyuz_address()
    test = valid_soyuz_address('testnet')

    assert is_valid_soyuz_address(main) is True
    assert is_valid_soyuz_address(main, 'mainnet') is True
    assert is_valid_soyuz_address(main, 'testnet') is False

    assert is_valid_soyuz_address(test) is False
    assert is_valid_soyuz_address(test, 'mainnet') is False
    assert is_valid_soyuz_address(test, 'testnet') is True


def test_invalid_soyuz_address():
    from soyuzlib import is_valid_soyuz_address

    main = invalid_soyuz_address()
    test = invalid_soyuz_address('testnet')

    assert is_valid_soyuz_address(main) is False
    assert is_valid_soyuz_address(main, 'mainnet') is False
    assert is_valid_soyuz_address(main, 'testnet') is False

    assert is_valid_soyuz_address(test) is False
    assert is_valid_soyuz_address(test, 'mainnet') is False
    assert is_valid_soyuz_address(test, 'testnet') is False
...
def test_parse_masternode_status_vin():
    from soyuzlib import parse_masternode_status_vin
...
def test_hash_function():
    ...
    hex_hash = "%x" % soyuzlib.hashit(sb_data_hex)
...
def test_blocks_to_seconds():
    ...
    assert Decimal(soyuzlib.blocks_to_seconds(0)) == Decimal(0.0)
    assert Decimal(soyuzlib.blocks_to_seconds(2)).quantize(precision) \
        == Decimal(314.4).quantize(precision)
    assert int(soyuzlib.blocks_to_seconds(16616)) == 2612035
